          ...
          <>
            ...
            <CardImgOverlay className="pe-none">
              ...
              <div className="pe-auto">
                <Badge className="fs-6 mb-2 shadow" pill>
                  {post.category?.name}
                </Badge>
              </div>
            </CardImgOverlay>
        ...
        <CardBody className="pt-1">
          ...
          {post.tags.length > 0 && (
            <div className="d-flex gap-2 mb-3 pt-2">
              {post.tags.map((t) => {
                return (
                  <Badge color="info" key={`tag-${t.id}`} pill>
                    {t.name}
                  </Badge>
                );
              })}
          ...
          <div className="d-flex flex-column align-items-end">
            <div className="d-flex flex-row flex-wrap gap-1">
              {post.reactions.map((r) => {
                if (
                  r.postReactions.some(
                    (pr) => pr.userProfileId == loggedInUser.id
                  )
                ) {
                  return (
                    <Button
            ...
            <div className="d-flex flex-row flex-wrap mt-3 w-100 gap-2">
              ...
              >{`${post.commentsCount} Comments`}</Button>
